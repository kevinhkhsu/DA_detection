...
if [ ! -f ${NET_FINAL}.index ]; then
  if [[ ! -z  ${EXTRA_ARGS_SLUG}  ]]; then
    CUDA_VISIBLE_DEVICES=${GPU_ID} time python ./tools/trainval_net_adapt_SST.py \
      --weight output/vgg16/${TRAIN_IMDB_S}/default/vgg16_faster_rcnn_iter_490000.pth \
      --imdb ${TRAIN_IMDB_S} \
      --imdbval ${TEST_IMDB} \
      --imdb_T ${TRAIN_IMDB_T} \
      --imdb_synth ${TRAIN_IMDB_SYNTH} \
      --iters ${ITERS} \
...
./experiments/scripts/test_adapt_faster_rcnn_.sh $@
