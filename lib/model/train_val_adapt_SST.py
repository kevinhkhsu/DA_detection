...
class SolverWrapper(object):
  ...
  def __init__(self, network, imdb, roidb, imdb_T, roidb_T, imdb_synth, roidb_synth, valroidb, output_dir, tbdir, pretrained_model=None):
    self.net = network
    self.imdb = imdb
    self.roidb = roidb
    self.imdb_T = imdb_T
    self.roidb_T = roidb_T
    self.imdb_synth = imdb_synth
    self.roidb_synth = roidb_synth
  ...
  def snapshot(self, iter):
    ...
    # current position in the database
    curSynth = self.data_layer_synth._cur
    # current shuffled indexes of the database
    permSynth = self.data_layer_synth._perm
    ...
    with open(nfilename, 'wb') as fid:
      ...
      pickle.dump(curSynth, fid, pickle.HIGHEST_PROTOCOL)
      pickle.dump(permSynth, fid, pickle.HIGHEST_PROTOCOL)
  ...
  def from_snapshot(self, sfile, nfile):
    ...
    with open(nfile, 'rb') as fid:
      ...
      self.data_layer_synth._cur = curSynth
      self.data_layer_synth._perm = permSynth
  ...
  def train_model(self, max_iters):
    ...
    self.data_layer_synth = RoIDataLayer(self.roidb_synth, self.imdb.num_classes)
    ...
    while iter < max_iters + 1:
      ...
      blobsSynth = self.data_layer_synth.forward()
      ...
      if iter % (cfg.TRAIN.DISPLAY) == 0:
        print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
              ...
              # '>>> ADAM_lr: %f' % \
              # '>>> recon_loss: %.6f\n >>> lr: %f' % \
              '>>> total loss: %.6f\n >>> rpn_loss_cls_synth: %.6f\n >>> rpn_loss_box_synth: %.6f\n >>> loss_cls_synth: %.6f\n >>> loss_box_synth: %.6f\n '
              '>>> D_img_loss_synth: %.6f\n >>> D_img2_loss_synth: %.6f\n' \
              % (iter, max_iters, total_loss, rpn_loss_cls, \
                ...
                loss_synth, rpn_loss_cls_synth, rpn_loss_box_synth, loss_cls_synth, loss_box_synth,
                loss_D_img_synth, loss_D_img2_synth))
...
def train_net(network, imdb, roidb, imdb_T, roidb_T, imdb_synth, roidb_synth, valroidb, output_dir, tb_dir,
  ...
  roidb_synth = filter_roidb(roidb_synth)

  sw = SolverWrapper(network, imdb, roidb, imdb_T, roidb_T, imdb_synth, roidb_synth, valroidb, output_dir, tb_dir,
